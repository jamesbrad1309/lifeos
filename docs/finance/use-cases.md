...
**Status**: ⬜ not built yet (nothing is, so far). Rows are sorted by
impact, highest first, and **Notes** link to the doc that designs each one.

## Accounts

| Status | Use case | Impact | Notes |
| :----: | -------- | :----: | ----- |
| ⬜ | **Create an account** (name, type: current / savings / credit card / cash / investment, opening balance) | 5 | `Account` model, see [data-model.md](data-model.md) |
...
| Status | Use case | Impact | Notes |
| :----: | -------- | :----: | ----- |
| ⬜ | **Log an expense or income** (amount, date, payee, category, note) | 5 | `createTransaction`, see [graphql-schema.md](graphql-schema.md) |
...
| Status | Use case | Impact | Notes |
| :----: | -------- | :----: | ----- |
| ⬜ | **Default category set** seeded on first run (Groceries, Rent, Transport, Eating out…) | 4 | `CategoriesService.seedDefaults()` on module init, see [backend-module.md](backend-module.md) |
...
| Status | Use case | Impact | Notes |
| :----: | -------- | :----: | ----- |
| ⬜ | **Monthly budget per category** | 4 | `Budget(categoryId, month)`, see [budgets-and-reports.md](budgets-and-reports.md) |
...
| Status | Use case | Impact | Notes |
| :----: | -------- | :----: | ----- |
| ⬜ | **Recurring transactions** (rent, salary, subscriptions) generated on schedule | 4 | Lazy, idempotent catch-up, see [recurring-and-import.md](recurring-and-import.md) |
...
| Status | Use case | Impact | Notes |
| :----: | -------- | :----: | ----- |
| ⬜ | **Create a goal** (target amount, optional deadline, linked account) | 3 | `SavingsGoal` model, see [data-model.md](data-model.md) |
...
| Status | Use case | Impact | Notes |
| :----: | -------- | :----: | ----- |
| ⬜ | **Spend by category this month** (donut or bar chart) | 4 | `spendByCategory(month)`, see [budgets-and-reports.md](budgets-and-reports.md) |
...
| Status | Use case | Impact | Notes |
| :----: | -------- | :----: | ----- |
| ⬜ | **CSV import** from a bank export with column mapping and duplicate detection | 4 | Column mapping + `importHash` dedupe, see [recurring-and-import.md](recurring-and-import.md) |
