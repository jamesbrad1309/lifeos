...
a ⬜. Within each table, rows are sorted by impact, highest first.
...
| ---- | :---: | :--: | ---------------------- |
| [Habit management](#habit-management) | 6 | 2 | Unarchive / view archived habits (2) |
| [Daily tracking](#daily-tracking) | 4 | 1 | Add a note to today's entry (2) |
| [Dashboard & gamification](#dashboard--gamification) | 3 | 1 | See longest streak / total completions per habit (2) |
| [Calendar](#calendar) | 2 | 1 | Week or month calendar view (2) |
| [History & review](#history--review) | 0 | 2 | View a single habit's full entry history (2) |
| [Motivation & rewards](#motivation--rewards) | 0 | 5 | Streak freeze (3) |
| [Routines & structure](#routines--structure) | 0 | 4 | Habit stacking / routines (3) |
| [Insights](#insights) | 0 | 4 | Best / worst weekday per habit (3) |
| [Journaling & mood](#journaling--mood) | 0 | 3 | Daily mood / energy check-in (3) |
| [Cross-module (habits × finance)](#cross-module-habits--finance) | 0 | 4 | "No-spend day" habit auto-checked from transactions (3) |

## Habit management

| Status | Use case | Impact | Notes |
| :----: | -------- | :----: | ----- |
| ✅ | **Create a custom habit** (name, icon/color, unit, target, schedule) | 5 | `Mutation.createHabit` |
...
| Status | Use case | Impact | Notes |
| :----: | -------- | :----: | ----- |
| ✅ | **See which habits are due today** | 5 | `Query.todayHabits`; day view applies the same `isDueOn` filter client-side |
...
| Status | Use case | Impact | Notes |
| :----: | -------- | :----: | ----- |
| ✅ | **See aggregate level/XP/streak stats across all habits** | 4 | `Query.dashboardStats`, `DashboardHeader` |
...
| Status | Use case | Impact | Notes |
| :----: | -------- | :----: | ----- |
| ✅ | **Day view: today's due habits on a time-of-day timeline** | 4 | `DayCalendar`, positions habits by `startTime`; untimed habits shown as an "Anytime" chip row |
...
| Status | Use case | Impact | Notes |
| :----: | -------- | :----: | ----- |
| ⬜ | **View a single habit's full entry history** (list, not just heatmap) | 2 | `Query.habitEntries(habitId)` exists — no history list UI |
...
| Status | Use case | Impact | Notes |
| :----: | -------- | :----: | ----- |
| ⬜ | **Streak freeze**: spend earned points to protect a streak on a missed day | 3 | A `StreakFreeze(habitId, date)` row; `streak.util.ts` treats a frozen day as "not due", like a paused habit |
...
| Status | Use case | Impact | Notes |
| :----: | -------- | :----: | ----- |
| ⬜ | **Habit stacking / routines**: group habits into an ordered "Morning routine" and check them off in sequence | 3 | `Routine` plus an ordered join table; the day view renders a routine as one block at its first habit's `startTime` |
...
| Status | Use case | Impact | Notes |
| :----: | -------- | :----: | ----- |
| ⬜ | **Best / worst weekday per habit** ("you skip gym on Fridays") | 3 | Group entries by `date.getDay()` over the stats window |
...
| Status | Use case | Impact | Notes |
| :----: | -------- | :----: | ----- |
| ⬜ | **Daily mood / energy check-in** (1–5) | 3 | Can be modelled as a built-in habit with `unit: "mood"`, so it reuses entries, heatmaps and streaks with no new tables |
...
| Status | Use case | Impact | Notes |
| :----: | -------- | :----: | ----- |
| ⬜ | **"No-spend day" habit auto-checked from transactions** | 3 | Finance upserts `HabitEntry`, see [habits-integration.md §1](../finance/habits-integration.md#1-no-spend-day-habit-auto-checked) |
